...
namespace Final_Lab_1
{
    class SpecialSavingsAccount : Account
    {
        ...
        override public void transaction(double amount)
        {
            ...
            if (trans >= 5)
            {
                Console.WriteLine("Sorry The maximum transaction time committed");
            }
            else
            {
                if (this.balance - amount < temp)
                {
                    Console.WriteLine("Sorry transaction can't be completed as the remaining balance is less then 20% of balance ");
                }
                else
                {
